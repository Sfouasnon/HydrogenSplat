...
**Settled in M0 (2026-09-13), from the fork's source and one real train:** `--export-name` is a template — `{iter}` is zero-padded to the digit count of `--total-train-iters`, so 40000 gives `export_02500.ply` … `export_40000.ply`, and `--export-path` resolves against the dataset's parent unless absolute. Resume is mechanically available: `--start-iter` moves only the loop start, the initial splats come from whatever `.ply` sits in the dataset folder (`init.ply` wins), so `hs train --resume-from` stages an export as `init.ply`; whether the result is as good as an uninterrupted run is untested, and M3 still decides between "resume" and "restart". Two things the run added that the plan did not anticipate: Brush prints no `Refine iter` line in the last ~2,000 iterations, so progress goes quiet for the final ~2.5 minutes (the engine emits a heartbeat), and throughput falls from 23 to 14 it/s as the model grows, so an early ETA reads ~40% short. The adb and ffmpeg questions are untouched — M1 and M5.
