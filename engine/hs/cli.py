...
Stages: ingest, select, solve, train, move (alias paths), prune, render, views; split is an
...
from .stages import archive, calibrate, cameras, exposure, grade, ingest, masks, merge, move, movepreview, phone, prune, render, replay, select, selftest, solve, split, tools, train, views
...
def build_parser():
    ...
    for mod in (ingest, select, solve, exposure, masks, train, archive, merge, split, move, prune, render, grade, views, cameras, movepreview, tools, calibrate, selftest, phone, replay):
        mod.add_parser(sub)
    seen = set()
